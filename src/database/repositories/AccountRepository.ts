...
export const AccountRepository = {
  ...
  async insert(data: InsertAccount): Promise<Account> {
    const db = await getDatabase();
    const [result] = await db.executeSql(
      `INSERT INTO accounts (name, type, balance, "limit", color, bank_name)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [data.name, data.type, data.balance, data.limit ?? null, data.color, data.bankName ?? null],
    );
  ...
  async update(id: number, data: Partial<InsertAccount>): Promise<void> {
    const db = await getDatabase();
    const fields: string[] = [];
